  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="Commissions"
        description="Commission rules and salesman commission calculations."
      />

      <nav
        aria-label="Commissions sections"
        ...
        <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:flex lg:min-w-0 lg:items-center gap-1 p-1">
          {visibleTabs.map((tab) => (
            <li key={tab.id} className="w-full lg:w-auto">
              ...
              >
                {tab.id === 'rules' && <Receipt className="h-4 w-4" aria-hidden="true" />}
                <span>{tab.label}</span>
              </NavLink>
            </li>
          ))}
