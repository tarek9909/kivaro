...
describe('SalesmanWorkspaceTab', () => {
  ...
  it('renders authoritative workspace figures with central order workflow links', async () => {
    renderWorkspace();
    ...
    expect(screen.getByText('Orders are managed centrally')).toBeInTheDocument();
  });

  it('lets an authorized admin select a salesman workspace', async () => {
    const user = userEvent.setup();
    api.pos.workspace.get
      .mockResolvedValueOnce({
        data: {
          workspace: {
            selection_required: true,
            available_salesmen: [{ id: 7, full_name: 'Maya Saleh' }]
          }
        }
      })
    ...
    await user.selectOptions(await screen.findByLabelText('Salesman'), '7');

    await waitFor(() => expect(api.pos.workspace.get).toHaveBeenLastCalledWith({ limit: 20, salesman_id: 7 }));
    expect(await screen.findByText('Maya Saleh')).toBeInTheDocument();
  });
});
