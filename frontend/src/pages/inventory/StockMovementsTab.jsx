...
import { Badge, Button, DataTable, EmptyState, Input, Pagination, Select } from '@/components/ui/index.js';
import { formatDateTime, formatNumber } from '@/lib/formatters.js';
import { INVENTORY_PERMISSIONS, MOVEMENT_TYPES } from './inventory.config.js';
...
function formatCartonChange(row) {
  const cartons = row.carton_count_change;
  const looseUnits = row.loose_units_change;
  if (cartons === null || cartons === undefined) {
    if (looseUnits === null || looseUnits === undefined) return '-';
    return `${formatNumber(looseUnits, { maximumFractionDigits: 0 })} loose`;
  }
  const cartonLabel = `${formatNumber(cartons, { maximumFractionDigits: 0 })} carton${Math.abs(Number(cartons)) === 1 ? '' : 's'}`;
  if (looseUnits === null || looseUnits === undefined || Number(looseUnits) === 0) return cartonLabel;
  return `${cartonLabel} · ${formatNumber(looseUnits, { maximumFractionDigits: 0 })} loose`;
}
...
export default function StockMovementsTab() {
  ...
  const queryParams = useMemo(() => {
    ...
    return params;
  }, [dateFrom, dateTo, debouncedSearch, itemId, limit, movementType, page, warehouseId]);
  ...
  const itemsQuery = useItemsOptions(canView && canLoadInventoryOptions);
    ...
    () => [
      ...
      },
      {
        ...
        cell: (row) => {
          ...
          return (
            <span className={`inline-flex items-center justify-end gap-1 font-mono text-sm ${isPositive ? 'text-emerald-200' : 'text-rose-200'}`}>
      ...
      },
      {
        ...
        align: 'right',
        cell: (row) => <span className="font-mono text-sm text-ink-100">{formatStockQuantity(row.reserved_quantity_after || 0, row)}</span>
      },
      {
        id: 'unit_cost',
        header: 'Unit cost',
        align: 'right',
        cell: (row) => row.unit_cost === null || row.unit_cost === undefined
          ? <span className="text-xs text-ink-400">-</span>
          : <span className="font-mono text-sm text-ink-100">{formatNumber(row.unit_cost, { maximumFractionDigits: 4 })}</span>
      },
      {
        id: 'reference',
        header: 'Reference',
        cell: (row) => row.reference_type
          ? <span className="font-mono text-xs text-ink-200">{row.reference_type}{row.reference_id ? ` #${row.reference_id}` : ''}</span>
          : <span className="text-xs text-ink-400">-</span>
      }
    ],
    []
  ...
  if (!canView) {
    return <EmptyState title="Permission required" description="Your role does not have stock.movements. Ask an administrator to grant it." />;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Select value={movementType} onChange={(event) => { setMovementType(event.target.value); setPage(1); }}>
      ...
      <DataTable
        ...
        empty={{ title: 'No movements match the filters', description: 'Adjust the filters or post a stock receipt or adjustment.' }}
