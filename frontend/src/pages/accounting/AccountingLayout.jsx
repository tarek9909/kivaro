...
import { ACCOUNTING_TABS } from './accounting.config.js';
import { cn } from '@/lib/cn.js';

export default function AccountingLayout() {
  const hasAnyPermission = useAuthStore((state) => state.hasAnyPermission);
  const hasModule = useAuthStore((state) => state.hasModule);
  const visibleTabs = ACCOUNTING_TABS.filter((tab) =>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Finance"
        title="Accounting"
        description="Expense categories, expenses, cash accounts, financial transactions, and salesman balances."
      />
