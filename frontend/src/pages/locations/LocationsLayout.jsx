...
import { LOCATIONS_TABS } from './locations.config.js';
import { cn } from '@/lib/cn.js';

export default function LocationsLayout() {
  const hasAnyPermission = useAuthStore((state) => state.hasAnyPermission);
  const hasModule = useAuthStore((state) => state.hasModule);
  const visibleTabs = LOCATIONS_TABS.filter((tab) =>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="Locations and targets"
        description="Manage sales territories, sublocations, salesmen, and target plans."
      />
